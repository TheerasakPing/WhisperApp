...
struct HistoryCoreTests {
    ...
    static func testRetentionPrunesExpiredRecords() throws {
        ...
        let document = HistoryPolicy.pruned(
            HistoryDocument(retention: .sevenDays, records: [old, fresh]),
            now: now,
            maxRecords: 100
        )
        try expect(document.records.map(\.finalText) == ["fresh"], "7-day retention should remove expired records")
    }
    ...
    static func testStoreCRUDAndLatest() throws {
        ...
        let latestAfterAppend = try store.latest(now: Date(timeIntervalSince1970: 20))
        try expect(latestAfterAppend?.id == second.id, "latest should return newest record")
        ...
        let latestAfterUpdate = try store.latest(now: Date(timeIntervalSince1970: 20))
        try expect(latestAfterUpdate?.finalText == "second edited", "updated text should persist")

        try store.delete(id: second.id)
        let latestAfterDelete = try store.latest(now: Date(timeIntervalSince1970: 20))
        try expect(latestAfterDelete?.id == first.id, "delete should remove selected record")

        try store.clear()
        let afterClear = try store.load(now: Date(timeIntervalSince1970: 20))
        try expect(afterClear.records.isEmpty, "clear should remove all records")
    }
