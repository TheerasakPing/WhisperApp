    ...
    func generate(
        ...
        ProviderFallbackRunner.run(providerIDs: chain, attempt: { providerID, done in
            ...
            guard let body = try? JSONSerialization.data(withJSONObject: spec.body) else {
                    ...
                    kind: .invalidRequest,
                    message: "Could not encode meeting notes request"
                )))
                return
            }
            ...
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                guard let data,
                      ...
                      let raw = LLMRequestBuilder.extractText(from: json, apiProtocol: spec.apiProtocol) else {
                    done(.failure(ProviderAttemptFailure(
                        kind: .invalidResponse,
                        message: "\(provider.name) returned invalid meeting notes"
                    )))
                    return
                }
                ...
                guard !value.isEmpty else {
                        ...
                        kind: .invalidResponse,
                        message: "\(provider.name) returned empty meeting notes"
                    )))
                    return
                }
                done(.success(value))
